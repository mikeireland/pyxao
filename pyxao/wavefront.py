...
class Wavefront():
    ...
    def __init__(self,wave=1.65e-6,m_per_pix=0.02,sz=512,pupil={'type':"annulus", 'dout':8.0,'din':3.0}):
        """Initialize the key parameters for the wavefront
    ...
    def pupil_field(self,atm,edge_smooth=16):
        ...
        for i in range(0,atm.nlayers):
            ...
            self.field = self.propagators[i].propagate(self.field)
            #print("t1: {0:6.3f}, t2 {1:6.3f}".format(toc-tic, time.time()-toc))
        self.field *= self.pupil 
        
    def image(self, return_efield=False):
        """Return an image based on the current field. Zero-pad in order to ensure nyquist
        sampling. """
