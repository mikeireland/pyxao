...
class ShackHartmann(WFS):
    ...
    def __init__(
        ...
        # WFS geometry...
        self.nlenslets_per_side = nlenslets_per_side
        self.plate_scale_as_px = plate_scale_as_px
        self.detector_sz = detector_sz
        self.geometry = geometry

        # If the number of lenslets is odd, then we will have a central lenslet.
        if nlenslets_per_side % 2 == 1:
        ...
        lenslet_pitch_px = self.wavefronts[0].sz / nlenslets_per_side  # lenslet size in pixels
        ...
        if geometry == 'hexagonal':
             nrows = np.int(np.floor(nlenslets_per_side / np.sqrt(3)/2))*2+1 #always odd
             xpx = np.tile(wavefronts[0].sz//2 + (np.arange(nlenslets_per_side) - nlenslets_per_side//2)*lenslet_pitch_px,nrows)
             xpx = np.append(xpx,np.tile(wavefronts[0].sz//2 - lenslet_pitch_px/2 + (np.arange(nlenslets_per_side) - nlenslets_per_side//2)*lenslet_pitch_px,nrows-1))
             ypx = np.repeat( wavefronts[0].sz//2 + (np.arange(nrows) - nrows//2)*np.sqrt(3)*lenslet_pitch_px,nlenslets_per_side)
             ypx = np.append(ypx,np.repeat( wavefronts[0].sz//2 -np.sqrt(3)/2*lenslet_pitch_px + (np.arange(nrows-1) - nrows//2+1)*np.sqrt(3)*lenslet_pitch_px,nlenslets))
             if not central_lenslet:
                ypx += lenslet_pitch_px/np.sqrt(3)

        # Square geometry        
        elif geometry == 'square':
            # xpx and ypx give the coordinates of each lenslet
            xpx = np.tile(   wavefronts[0].sz//2 + (np.arange(nlenslets_per_side) - nlenslets_per_side//2)*lenslet_pitch_px,nlenslets_per_side)
            ypx = np.repeat( wavefronts[0].sz//2 + (np.arange(nlenslets_per_side) - nlenslets_per_side//2)*lenslet_pitch_px,nlenslets_per_side)
            if not self.central_lenslet:
                xpx += lenslet_pitch_px/2
                ypx += lenslet_pitch_px/2
        else:
        ...
        px = px[good]
        self.nlenslets_total = px.shape[0]    

        # Coordinates of the lenslets in the wave grid.
        self.wave_px = px 
        # Coordinates of the lenslets in the detector grid.
        self.detector_px = self.wave_px * \
            self.detector_sz / self.wavefronts[0].sz 
        if plotit:
            #plt.clf()
            plt.figure()
            plt.plot(px[:,0], px[:,1],'o')
            plt.title('Lenslet locations')
            plt.show()
        ...
        self.nsense = 3*self.nlenslets_total #x, y and flux
        ...
        for wf in wavefronts:
            ...
            if geometry == 'hexagonal':
                one_lenslet *= ot.utils.hexagon(wf.sz,lenslet_pitch_px)
            elif geometry == 'square':
                one_lenslet *= ot.utils.square(wf.sz,lenslet_pitch_px)
            else:
                print("ERROR: invalid wavefront sensor geometry")
                raise UserWarning

            # Duplicating the wavefronts, one over each lenslet.
            for i in range(self.nlenslets_total):
        ...
        # Get the wavefront sensed by the WFS. At this point the wavefronts have
        # been reset and so all spots lie at the centre of their subapertures. 
        # This variable is used in sense() if the desired centroid output format
        # to be specified w.r.t. their nominal positions.
    ...
    def sense(self,mode='gauss_weighted',
        ...
        TODO: modify this so that it doesn't change the field variable in the 
        wavefront instances...
        ...
        """
        self.im = np.zeros( (self.detector_sz, self.detector_sz) )

        # Compute the image appearing on the WFS detector.
        for i in range(len(self.wavefronts)):
            # Make a temporary copy of the field so that we can restore it after 
            # propgation.
            if restore_field:
        ...
        window_hw = np.round((self.detector_sz / self.nlenslets_per_side) / 2)
        xx = np.arange(2*window_hw + 1) - window_hw
        xy = np.meshgrid(xx,xx)
        #  xyf[0] = x coord; xyf[1] = y coord; xyf[2] = image sum (denominator)
        xyf = np.empty( (3,self.nlenslets_total) )    

        for i in range(self.nlenslets_total):
            x_int  = int(np.round(self.detector_px[i][0]))
            x_frac = self.detector_px[i][0] - x_int
            y_int  = int(np.round(self.detector_px[i][1]))
            y_frac = self.detector_px[i][1] - y_int
            
            subim = self.im[y_int-window_hw:y_int+window_hw+1,\
            x_int-window_hw:x_int+window_hw+1]
...
class OldShackHartmann(WFS):
    def __init__(
            ...
            lenslet_pitch_px = lenslet_pitch_m/wavefronts[0].m_per_px
            nlenslets = int(np.floor(wavefronts[0].sz/lenslet_pitch_px))

            # Hexagonal geometry
            if geometry == 'hexagonal':
                 nrows = np.int(np.floor(nlenslets / np.sqrt(3)/2))*2+1 #always odd
                 xpx = np.tile(wavefronts[0].sz//2 + (np.arange(nlenslets) - nlenslets//2)*lenslet_pitch_px,nrows)
                 xpx = np.append(xpx,np.tile(wavefronts[0].sz//2 - lenslet_pitch_px/2 + (np.arange(nlenslets) - nlenslets//2)*lenslet_pitch_px,nrows-1))
                 ypx = np.repeat( wavefronts[0].sz//2 + (np.arange(nrows) - nrows//2)*np.sqrt(3)*lenslet_pitch_px,nlenslets)
                 ypx = np.append(ypx,np.repeat( wavefronts[0].sz//2 -np.sqrt(3)/2*lenslet_pitch_px + (np.arange(nrows-1) - nrows//2+1)*np.sqrt(3)*lenslet_pitch_px,nlenslets))
                 if not central_lenslet:
                    ypx += lenslet_pitch_px/np.sqrt(3)
            ...
            # Square geometry        
                ...
                # xpx and ypx give the coordinates of each lenslet
                xpx = np.tile(   wavefronts[0].sz//2 + (np.arange(nlenslets) - nlenslets//2)*lenslet_pitch_px,nlenslets)
                ypx = np.repeat( wavefronts[0].sz//2 + (np.arange(nlenslets) - nlenslets//2)*lenslet_pitch_px,nlenslets)
                if not central_lenslet:
                    xpx += lenslet_pitch_px/2
                    ypx += lenslet_pitch_px/2
            else:
