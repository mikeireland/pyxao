...
wf = wavefront.Wavefront(wave=850e-9,m_per_px=m_per_px,pupil=pupil,sz=wave_height_px)

# Atmosphere. Keep default values for now
atm = atmosphere.Atmosphere(m_per_px=m_per_px)
...
# Need to make sure it's not possible to input conflicting parameters - for now stick with N_os, detector size and # lenslets 

# OK - given N_os, detector_sz and nlenslets, how do we calculate the focal length/F#?
# plate_scale_as_px from N_os at the given wavelength
# (note: will have to specify Nyquist sampling AT a wavelength! so might be better to put in plate scale instead)
...
# Note: all of these dimensions have been scaled up to the size of the primary aperture.
l_px_m = pupil['dout'] / detector_sz
plate_scale_as_mm = plate_scale_as_px / l_px_m
flength_m = 206265 / plate_scale_as_mm
